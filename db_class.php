...
class db {
    ...
    function getCitySensors($city) {
        ...
        $result = $this->db->query("SELECT s.id, s.name, s.substances, count(sd.timestamp), s.country as cnt FROM {$this->db_table} s, {$this->data_table_month} sd WHERE city = '" . $city . "' AND s.id = sd.sensor_id GROUP BY sd.sensor_id ORDER BY cnt DESC;");
        return $result;
    }
    ...
    function getSensorInfo($sensor_id) {
        $result = $this->db->query("SELECT city, name, substances, country FROM {$this->db_table} WHERE id = " . $sensor_id . ";");
        return $result;
    }
